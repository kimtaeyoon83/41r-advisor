...
from modules import cohort_runner
...
def test_browser_session_succeeds_first_try():
    ...
    with patch("modules.agent_loop.run_session", return_value=mock_log) as mock_run:
...
def test_browser_session_retries_on_failure():
    ...
    with patch("modules.agent_loop.run_session", side_effect=Exception("boom")) as mock_run:
...
def test_browser_session_succeeds_after_retry():
    ...
    with patch("modules.agent_loop.run_session", side_effect=side_effects) as mock_run:
        result = cohort_runner._run_browser_session("p_test", "https://example.com", "task", max_retries=2)
