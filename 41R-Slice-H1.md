...
**H1 가설 (v2, 2026-04-14 피벗)**: CPO/Head of Growth는 **A/B 테스트 설계 전 단계**의 "세그먼트 분기 진단"에 **건당 ~$5 수준 예산**을 낼 의향이 있다.

(이전 v1: "유료 전환율 5%" 추상적 → v2: 구체적 운영 가격대 + 사용 시점 명시)

**단계화된 Kill Criteria** (빠른 중간 신호로 분리):

| 단계 | 임계 (Kill) | 임계 (Pass) | 액션 |
|---|---|---|---|
| 발송 → 응답 | 30건 발송 중 응답 < 3건 | 응답 ≥ 5건 | Kill = 메시징 재설계 / Pass = 다음 단계 |
| 응답 → 진단 샘플 요청 | 응답 5건 중 요청 0 | **요청 ≥ 3건** | Kill = 가치 제안 재설계 / Pass = 샘플 발송 |
| 샘플 → 미팅 | 요청 3건 중 미팅 0 | 미팅 ≥ 2건 | Kill = 샘플 품질 재검토 / Pass = 미팅 진행 |
| 미팅 → 유료 의향 | 미팅 2건 중 의향 0 | 의향 ≥ 1건 | Kill = 가격/스코프 재설계 / Pass = 결제 유도 |
| 의향 → 결제 | $5 이상 결제 0 | 1건+ 결제 | Kill = H2 이하 중단 / Pass = H2 진입 |

각 단계 cut-off가 "**다음 단계 진행 vs 메시징/포지셔닝 재설계**" 결정점.
